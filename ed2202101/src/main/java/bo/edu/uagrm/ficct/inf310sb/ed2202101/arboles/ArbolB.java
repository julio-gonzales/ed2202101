...
//NO ESTAMOS ACTUALIZANDO LA RAIZ POR ESO ES EL PROBLEMA
        ...
        while (!NodoMVias.esNodoVacio(nodoActual)) {
            if (nodoActual.cantidadDeClavesNoVacias() > nroMaximoDeDatos) {
                ...
                for (int i = (this.nroMaximoDeDatos /2) + 1; i < orden; i++) {
                    nuevoHijoDerecho.setClave(contadorDelNuevoNodoDerecho,nodoActual.getClave(i ));
                    nuevoHijoDerecho.setValor(contadorDelNuevoNodoDerecho, nodoActual.getValor(i ));
                ...
                for (int i = posicionDeClaveEnNodoPadre; i < orden; i++) {
                    ...
                    nodoPadre.setHijo(i + 1, nuevoHijoDerecho);
    ...
    //11. Implemente el método eliminar del árbol b.
    @Override
    ...
    private void prestarOFusionar(NodoMVias<K,V> nodoActual, Stack<NodoMVias<K,V>> pilaDeAncestros) {
        ...
        if (NodoMVias.esNodoVacio(nodoPadre.getHijo(0))) {
    ...
    private NodoMVias<K,V> buscarNodoDeClavePredecesora(Stack<NodoMVias<K,V>> pilaDeAncestros, NodoMVias<K,V> nodoActual) {
    ...
    private NodoMVias<K,V> buscarNodoDeLaClave(K claveAEliminar, Stack<NodoMVias<K,V>> pilaDeAncestros) {
        NodoMVias<K,V> nodoActual = this.raiz;
        while (NodoMVias.esNodoVacio(nodoActual)) {
            boolean noBajoPorAlgun = true;
            for (int i = 0; i < nodoActual.cantidadDeClavesNoVacias() && noBajoPorAlgun; i++) {
